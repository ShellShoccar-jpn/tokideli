...
指定した単位時間あたり1行のみ通過できるように流速制限します。書式は`n[.ddddddddd][s|ms|us|ns]`であり、数値部`n[.ddddddddd]`と単位部`[s|ms|us|ns]`から成ります。
