...
このコマンドが便利なのはIoTやM2Mと呼ばれる分野です。例えば、ポテンショメーターを操作し、値が変化する時だけデータを出力するデバイスがあったとします。欲しい値は、摺動子（ツマミ）をいじっている最中のものではなくて、いじり終えて確定した値だけということがしばしばありますが、そのような場面でこのコマンドが役立ちます。すなわち、そのデバイスのデータを行指向のテキストデータに変換したうえでこのコマンドに与えれば、最終的に確定した値だけが標準出力から得られます。
